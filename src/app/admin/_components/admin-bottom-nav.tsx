...
export function AdminBottomNav() {
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-[460px] grid-cols-5 gap-1 px-2 py-2">
        {tabs.map((tab) => (
          <Link
            key={`${tab.href}-${tab.label}`}
            href={tab.href}
            className={`flex min-h-11 touch-manipulation items-center justify-center rounded-xl px-1 text-center text-[11px] font-medium leading-tight tracking-[0.01em] sm:text-xs ${
              tab.active
